import React, { Component } from 'react'


export class NewsItem extends Component {
  render() {
    let {title,decription,imageUrl,newsUrl,date,author,source} = this.props;
    return (
      <div>
          <div className={`card ${this.props.darkMode?"bg-dark text-white" : "bg-white text-dark"}`}>
          <span className="position-absolute start-0 badge bg-danger" style={{top:"-12px"}}>
            {source}
          </span>
            <img src={imageUrl?imageUrl:"data:image/png;base64,"} className="card-img-top" alt="..."/>
            <div className="card-body">
              <h5 className="card-title">{title}</h5>
              <p className="card-text">{decription}</p>
              <p className="card-text"><small className={`${this.props.darkMode?"text-secondary" : "text-body-secondary"}`}>By {author?author:"unknown"} On {new Date(date).toGMTString()}</small></p>
              <a href={newsUrl} target='_blank' rel="noreferrer" className={`btn ${this.props.darkMode?"btn-secondary" : "btn-dark"}`}>Read more</a>
            </div>
          </div>
      </div>
    )
  }
}

export default NewsItem
